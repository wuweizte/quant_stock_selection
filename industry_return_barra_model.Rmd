...
```{r}
latest.pettm <- monthly.pettm.data[,"d20180130"]
quantile(latest.pettm, na.rm = TRUE)
```
...
```{r}
# preProcValues <- preProcess(monthly.price.data, method = c("center"))
# 
# trainTransformed <- predict(preProcValues, monthly.price.data)
# 
# all.equal(rtn, trainTransformed)

# all.equal(rtn, matrix(1,nrow(rtn), ncol(rtn)))
