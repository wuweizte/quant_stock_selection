---
title: "fcfftomv_factor_efficiency_test_in_seven_factor_model"
author: "Wu Wei"
date: "2018-1-25"
...
standardlized.monthly.fcfftomv <- 
        read.csv(
        "D:\\MyR\\quantprog\\forallAstock\\Bstandardlizefactor\\standardlized.monthly.fcfftomv.csv", 
        stringsAsFactors = FALSE)
...
# for(i in 1:1){
        ...
        ind.dum <- cbind(ind.dum,
                         ...
                         abcdefgh.fcfftomv = standardlized.monthly.fcfftomv[,(i + 1)])
...
```{r}
write.csv(result.F.hat.g, file = "result.F.fcfftomv.csv")
```

```{r}
t.test(tail(result.F.hat.g["fcfftomv",], 24))
```

```{r}
t.test(result.F.hat.g["fcfftomv",])

# forecast(ets(result.F.hat.g["fcfftomv",]), h = 2)
# 
# forecast(ets(head(result.F.hat.g["fcfftomv",], 120)), h = 2)

# auto.arima(result.F.hat.g["fcfftomv",])
```


```{r}
plot(result.F.hat.g["fcfftomv",], type = "l")
abline(h=0,col = "red")
```
