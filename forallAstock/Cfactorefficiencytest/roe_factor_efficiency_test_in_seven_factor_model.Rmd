---
title: "roe_factor_efficiency_test_in_seven_factor_model"
author: "Wu Wei"
date: "2018-1-25"
...
standardlized.monthly.roe.data <- 
        read.csv(
        "D:\\MyR\\quantprog\\forallAstock\\Bstandardlizefactor\\standardlized.monthly.roe.data.csv", 
        stringsAsFactors = FALSE)
...
```{r}
# write.csv(result.F.hat.g, file = "result.F.csv")
```

```{r}
t.test(tail(result.F.hat.g["roe",], 24))
```

```{r}
t.test(result.F.hat.g["roe",])

forecast(ets(result.F.hat.g["roe",]), h = 2)

forecast(ets(head(result.F.hat.g["roe",], 120)), h = 2)

auto.arima(result.F.hat.g["roe",])
```


```{r}
plot(result.F.hat.g["roe",], type = "l")
abline(h=0,col = "red")
```
