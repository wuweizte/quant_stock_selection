...
load("result.investion.target.name")
```

```{r}
write.csv(result.investion.target.name,file = "result.investion.name.csv")
```
...
```{r}
sum(selected.stocks.actual.return)
sum(hushen300.return)
```

```{r}
options(width = 103)
print(round(selected.stocks.actual.return,digits = 4))
print(round(hushen300.return,digits = 4))
```
